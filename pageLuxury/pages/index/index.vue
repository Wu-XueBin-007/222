  ...
  <mescroll-body ref="mescrollRef" :sticky="true" @init="mescrollInit" :down="{ native: true }" @down="downCallback"
	...
	<view class="banr">
		...
		<u-popup v-model="showOrder" mode="bottom"  @close="close" @open="open">
			<view class="" style="font-size: 36upx;color: #404040;text-align: center;padding: 20upx 0;font-weight: bold;">
				专区订单
			</view>
								...
								<view class="specialOrderItemBRB">
									<view class="specialOrderItemBRBL" >
										<view class="" v-if="item.teamStat&&item.is_free==1">
											<image :class="[indexs!=0?'Image':'']" v-for="(items,indexs) in item.team.list" :key="indexs" :src="items.user.avatar_url" mode=""></image>
											<image class="Image" v-if="indexs<4" src="../../static/ask.png" mode=""></image>
										
										</view>
										<view class="" v-if="!item.teamStat&&item.is_free==1">
											<image :class="[indexs!=0?'Image':'']" v-for="(items,indexs) in 4" :key="indexs" src="../../static/ask.png" mode=""></image>
										</view>
									</view>
									
								<!-- v-if="item.is_free==1&&item.is_inviter==1" -->
									<!-- #ifdef MP-WEIXIN -->
									<button open-type="share"v-if="item.is_free==1&&item.is_inviter==1" class="shareBtn" >
									...
									<view class="specialOrderItemBRBR1" v-if="item.is_free==1&&item.is_back==0&&item.is_inviter==0">
										待返现
									</view>
								</view>
							</view>
	...
	<view class="advertisement2" v-else>
		...
		<view class="advertisementR">
			<view class="advertisementRT">
				<progress :percent="pgList" border-radius="6" backgroundColor="#FFF4E1" activeColor="#D99B5F" stroke-width="4" />
			</view>
        ...
        <view v-else class="multi">
          ...
          <view class="detail">
            ...
            <view class="detail-price onelist-hidden" style="margin-top: 18rpx;">
              <text class="goods-price f-30 col-m">￥ <text style="font-size: 36rpx;font-weight: bold;">{{ item.goods_price_min }}</text> </text>
	...
	<view class="ranks" v-if="isRanks">
		...
		<view class="ranksR">
			<image v-for="(item,index) in team.list" :kry="index" class="potL" :src="item.user.avatar_url" mode=""></image>
...
<script>
  ...
  export default {
    ...
    methods: {
		...
		getbigvip(){
			...
			memberApi.index(obj).then(res=>{
				...
				let possces = 0
				process = Math.ceil(Number(this.bigUser.consumption)/Number(this.setting.Luxury_upgrade_consum)*100)
				this.pgList = process
				console.log(Number(this.bigUser.consumption),Number(this.setting.Luxury_upgrade_consum),process)
    ...
    onShareAppMessage() {
      ...
      return {
        title: "高奢名品",
        path: "/pageLuxury/pages/index/index?" + this.$getShareUrlParams()+"&group_order_id="+this.group_order_id
      }
    },
