...
$http.requestStart = options => {
	// //console.log(requestNum,options.load,123123)
	if (options.load) {
		if (requestNum <= 0) {
			// 打开加载动画
			uni.showLoading({
	...
	// options.header['platform'] = "MP-WEIXIN"
	// 请求前加入Token
	options.header['Access-Token'] = store.getters.token
	if (options.method == "POST" && store.getters.platform == "App") {
...
$http.requestEnd = options => {
	...
	if (options.load) {
		...
		// //console.log(requestNum,options.load,321321)
		if (requestNum <= 0) {
