				...
				<view class="item-wrapper" :class="['color-' + color[index % color.length]]">
					...
					<view class="content dis-flex flex-dir-column flex-x-between">
						...
						<view class="bottom dis-flex flex-y-center">
							...
							<view class="receive" style="width: auto;padding: 0 16upx;line-height: 42upx;" v-if="item.state.value" @click="receive(item.coupon_id)">
								仅需{{item.integral}}积分
							</view>
							<view v-else class="receive state">
								<text>{{ item.state.text == "已领取" ? "已兑换" : item.state.text }}</text>
							</view>
						</view>
					</view>
				</view>
