				...
				<mescroll-body ref="mescrollRef" :sticky="true" @init="mescrollInit" :down="{ native: true }" :height="100"
				  ...
				  <view class="exchange-content-item" v-for="(item,index) in list.data" :key="index">
				  	...
				  	<view class="exchange-content-right">
				  		...
				  		<view class="exchange-content-right-sure" @click="toSettle" :data-index="index" v-if="item.type == 1">兑换</view>
						<view class="exchange-content-right-status" v-if="item.type == 2">待发货</view>
						<view class="exchange-content-right-sure" @click="receipt" :data-index="index" v-if="item.type == 3">确认收货</view>
						<view class="exchange-content-right-status" v-if="item.type == 4">已完成</view>
				  	</view>
				  </view>
				</mescroll-body>
			</view>
...
<script>
	...
	import * as giftApi from "@/api/gift/index.js";
	const pageSize = 15
	export default {
		components: {
		  MescrollBody
		...
		methods: {
			...
			getOrderList(pageNo = 1) {
			  ...
			  return new Promise((resolve, reject) => {
			    giftApi.list({ page: pageNo }, { load: false })
			      .then(result => {
					  //console.log(result)
			        // 合并新数据
			...
			toList(){
				this.$navTo("pageHome/giftOrder/index")
			},
			toSettle(e){
				let index = e.target.dataset.index || e.currentTarget.dataset.index;
				uni.setStorageSync("giftGoods",[this.list.data[index]])
				this.$navTo("pages/checkout/index",{mode:'gift'})
			}
		}
	}
</script>
