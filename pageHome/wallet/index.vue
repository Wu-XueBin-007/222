<template>
	<view class="container" v-if="!isLoading" style="height: 100vh;">
		<head-nav title="我的钱包" :backFlag="true" color="white" backGround="linear-gradient(90deg, #FF5F60, #DE6BAD)"
		...
		<uni-datetime-picker v-show="showModel" v-model="range" type="daterange" start="1990-01-01" end="2999-01-01"
			rangeSeparator="至" @change="changeDate" />
		<view class="infoWrap">
		...
		<swiper :indicator-dots="false" :autoplay="false" :style="{'height': 'calc(100% - ' + heights + 'px)'}"
			:current="navIndex" @change="changeDates">
			<swiper-item style="height: 100%;" v-for="index of 5" :key="index">
				<scroll-view scroll-y="true" @scrolltolower="loadMore"
					style="height: 100%;padding-top: 20rpx;box-sizing: border-box;padding-bottom: 20rpx;"
					v-if="list.length>0">
					<view class="msgItem" v-for="(item,index) in list" :key="index">
						<view class="msgItemL">
							<view class="msgItemLL msgItemLPT"
								v-if="item.scene==90||item.scene==100||item.scene==110||item.scene==120||item.scene==130">
								拼</view>
							<view class="msgItemLL msgItemLMS" v-if="item.scene==150||item.scene==160||item.scene==170">
								秒</view>
							<view class="msgItemLL msgItemLCP" v-if="item.scene==65||item.scene==50||item.scene==60">车
							</view>
							<view class="msgItemLL msgItemLSC" v-if="item.scene==20||item.scene==40||item.scene==200">商
							</view>
							<view class="msgItemLL msgItemLSM" v-if="item.scene==250||item.scene==260||item.scene==270">
								盲</view>
							...
							<view class="msgItemLL msgItemLHF" v-if="item.scene==70||item.scene==80||item.scene==75">话
							</view>
						...
						nodes1.boundingClientRect(cur1 => {
							...
							if (cur1 && cur1.height) {
								nodes2.boundingClientRect(cur2 => {
									...
									if (cur2 && cur2.height) {
										if (this.navIndex == 4) {
											let heights = cur.height + cur1.height + cur2.height +
												App.globalData.navH + 42;
