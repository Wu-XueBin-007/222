...
<script>
  ...
  import { wxPayment,zfbPayment } from '@/utils/app'
		  ...
		  uni.showActionSheet({
			...
			success(res) {
				...
				if(res.tapIndex == 0){
					...
					app.disabled = true
					// 提交到后端
					let obj = {planId: app.selectedPlanId, customMoney: app.inputValue,pay_mode:1}
					// #ifdef APP-PLUS
					obj.pay_mode = 3;
					// #endif
					RechargeApi.submit(obj)
					  .then(result => app.wxPayment(result.data.data))
					  .finally(() => app.disabled = false)
				}else if(res.tapIndex == 1){
					// 按钮禁用
					app.disabled = true
					// 提交到后端
					RechargeApi.submit({ planId: app.selectedPlanId, customMoney: app.inputValue,pay_mode:2 })
					  .then(result => app.zfbPayment(result.data.data))
					  .finally(() => app.disabled = false)
				}
			}
		  })
