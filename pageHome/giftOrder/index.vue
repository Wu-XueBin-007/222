        ...
        <view class="order-item" v-for="(item, index) in list.data" :key="index">
          ...
          <view v-if="item.order_status != 20" class="order-handle">
            <view class="btn-group clearfix" style="display: flex;justify-content: flex-end;">
			  ...
			  <view style="margin-left: 20upx;" v-if="item.order_status != OrderStatusEnum.APPLY_CANCEL.value && item.delivery_type == 20 && item.pay_status == PayStatusEnum.SUCCESS.value && item.delivery_status == DeliveryStatusEnum.NOT_DELIVERED.value">
...
<script>
  ...
  export default {
    ...
    methods: {
	  ...
	  onCode(orderId){
		  this.$navTo('pageHome/giftOrder/orderCode',{orderId:orderId})
	  },
      ...
      handleTargetDetail(orderId) {
        this.$navTo('pageHome/giftOrder/detail', { orderId })
      },

      // 跳转到订单评价页
      handleTargetComment(orderId) {
        this.$navTo('pageHome/giftOrder/comment/index', { orderId })
      }
