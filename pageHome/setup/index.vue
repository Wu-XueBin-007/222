<template>
	<view class="setUp">
			...
			<view class="setUp-item-footer" @click="btn_userInfo">
				...
				<view class="setUp-yhq">
					<view class="setUp-head-icon1">
						<image :src="userInfo.avatar?userInfo.avatar.preview_url:'../../static/default-avatar.png'">
						</image>
					</view>
				</view>
			</view>
		...
		<view v-if="isLogin" class="setUp-buttom" @click="btn_signOut">
			退出登录
		</view>
		<view
			style="position: fixed;top: 0;left: 0;right: 0;bottom: 0;width: 100%;height: 100%;background: #F5F5F5;z-index: -1;">
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			btn_certifindMember() {
				this.$navTo("pageHome/setup/editName", {
					...
					success(res) {
						uni.uploadFile({
							...
							success: (rep) => {
								...
								UserApi.edit_avatar({
										avatar_id: data.data.fileInfo.file_id
									})
									.then(res => {
