<template>
	<view class="withrawal">
		<head-nav title="提现记录" :backFlag="true" color="white" backGround="linear-gradient(90deg, #FF5F60, #DE6BAD)" backType="other" fontSize="36"></head-nav>
		<view class="con" v-if="hisList.length>0">
			<view class="conItem" v-for="(item,index) in hisList" :key="index">
				...
				<view class="conItemL" style="flex: 1;">
					<view class="conItemLT conItemRBorder" style="text-align: right;width: 100%;font-size: 28rpx;font-family: PingFang;font-weight: 500;color: #FF5F60;line-height: 28rpx;">{{item.price}}</view>
		...
		methods: {
			...
			getOrderList(pageNo = 1) {
			  ...
			  return new Promise((resolve, reject) => {
			    API.withdrawalHis({ if_commission:app.type, page: pageNo }, { load: false })
			      .then(result => {
			        // 合并新数据
