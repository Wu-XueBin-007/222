			...
			<view class="order-list">
				<view class="order-item" v-for="(item, index) in list.data" :key="index" style="position: relative;">
					<view class="item-top">
						<view class="item-top-left" style="display: flex;align-items: center;">
							<image src="../../static/home/seckill_order_icon.png"
								v-if="item.is_big_vip==0&&item.order_source==20" style="width: 52rpx;z-index: 9;"
								mode="widthFix"></image>
							<image src="../../static/home/lucky_order_icon.png"
								v-else-if="item.is_big_vip==0&&item.order_source==40" style="width: 52rpx;z-index: 9;"
								mode="widthFix"></image>
							<image src="../../static/home/BigMember.png" v-else-if="item.is_big_vip==1"
								style="width: 52rpx;z-index: 9;" mode="widthFix"></image>
							<image src="../../static/home/huxury.png" v-else-if="item.is_luxury_goods==1"
								style="width: 52rpx;z-index: 9;" mode="widthFix"></image>
							<image src="../../static/home/product_order_icon.png" v-else
								style="width: 52rpx;z-index: 9;" mode="widthFix"></image>
              ...
              <text class="state-text">{{ item.state_text }}</text>
					...
					<view class="order-total">
						<text>共{{ item.total_num }}件商品，总价</text>
						<text class="unit">￥</text>
					...
					<view v-if="item.order_status != 20" class="order-handle">
						<view class="btn-group clearfix" style="display: flex;justify-content: flex-end;">
							...
							<view
								v-if="(item.order_status != OrderStatusEnum.APPLY_CANCEL.value && item.order_status != OrderStatusEnum.COMPLETED.value)||item.pay_status == 10"
								style="margin-right: 20upx;">
								<view
									v-if="(item.pay_status == PayStatusEnum.SUCCESS.value && item.delivery_status == DeliveryStatusEnum.NOT_DELIVERED.value)||item.pay_status == 10">
									<view class="btn-item" @click="editAddress(item.order_id)">修改地址</view>
								</view>
							</view>
							<!-- 已支付取消订单 -->
							<block
								v-if="item.order_status != OrderStatusEnum.APPLY_CANCEL.value && item.order_status != OrderStatusEnum.COMPLETED.value">
								<view
									v-if="(item.pay_status == PayStatusEnum.SUCCESS.value && item.delivery_status == DeliveryStatusEnum.NOT_DELIVERED.value) || (item.delivery_status == DeliveryStatusEnum.DELIVERED.value && item.receipt_status == ReceiptStatusEnum.NOT_RECEIVED.value)">
							...
							<view
								v-if="item.order_status == OrderStatusEnum.APPLY_CANCEL.value && item.order_status != OrderStatusEnum.COMPLETED.value"
								class="f-28 col-8">退款申请中</view>
							<view style="margin-left: 20upx;"
								v-if="item.order_status != OrderStatusEnum.COMPLETED.value && item.order_status != OrderStatusEnum.APPLY_CANCEL.value && item.delivery_type == 20 && item.pay_status == PayStatusEnum.SUCCESS.value && item.delivery_status == DeliveryStatusEnum.NOT_DELIVERED.value">
							...
							<!-- 查看物流 -->
							<view
								v-if="item.delivery_status == DeliveryStatusEnum.DELIVERED.value && item.receipt_status == ReceiptStatusEnum.NOT_RECEIVED.value && item.order_status != OrderStatusEnum.APPLY_CANCEL.value"
...
<script>
	...
	export default {
		...
		methods: {
			...
			onCode(orderId) {
				this.$navTo('pageHome/order/orderCode', {
					orderId: orderId
				})
			},
			...
			handleTargetDetail(orderId) {
				console.log(orderId)
				this.$navTo('pageHome/order/detail', {
					orderId
				})
			},
			...
			handleTargetComment(orderId) {
				this.$navTo('pageHome/order/comment/index', {
					orderId
				})
			}
