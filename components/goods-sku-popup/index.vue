  ...
  <view class="goods-sku-popup popup" catchtouchmove="true" :class="(value && complete) ? 'show' : 'none'"
    ...
    <view class="layer attr-content" :style="'border-radius: '+borderRadius+'rpx '+borderRadius+'rpx 0 0;'">
        ...
        <scroll-view class="specification-wrapper-content" scroll-y="true">
          <view class="specification-header">
            ...
            <view class="specification-right">
              ...
              <view class="inventory"> <text style="color: #333333;font-weight: bold;">{{ stockText }}</text> ：{{ selectShop[stockName] || defaultStock }}</view>
              <view class="choose" v-show="goodsInfo[specListName] && goodsInfo[specListName][0].name !== defaultSingleSkuName">
                <text v-if="!selectArr.every(val => val == '')"> <text style="color: #333333;font-weight: bold;">已选：</text> {{ selectArr.join(' ') }}</text>
              </view>
            </view>
          </view>
          ...
          <view class="specification-content">
            ...
            <view style="display: flex;">
              <view style="flex: 1;">
                <text style="font-size: 28rpx; color: #333; line-height: 50rpx; font-weight: bold;">数量</text>
              </view>
      ...
      <view class="btn-wrapper" v-else-if="mode == 1">
        <view class="sure3 add-cart" style="" @click="addCart" :style="'color:'+addCartColor+';background:'+addCartBackgroundColor">{{ addCartText }}</view>

        <view class="sure2" style="" @click="buyNow" :style="'color:'+buyNowColor+';background-color:'+buyNowBackgroundColor">{{ buyNowText }}</view>
      </view>
      ...
      checkInpath(clickIndex) {
        ...
        for (let i = 0, len = specList.length; i < len; i++) {
          ...
          for (let j = 0; j < len2; j++) {
			...
			console.log(choosed_copy2,"```````````````````````````",that.shopItemInfo,"```````````````````",choosed_copy)
