	...
	<view style="padding-bottom: 98upx;">
		...
		<view class="" style="width: 750upx;background-color: #F23A3A;padding: 24upx;">
			<view class="collageUser">
				<view class="userInfo">
					<view :class="['userInfoItem',info.record.length>index?'active':'']" v-for="(item,index) in info.num" :key="index" :style="{'background-image':info.record.length>index?'url('+info.record[index].user.avatar_url+')':''}">
						<image src="../../static/home/ask.png" class="userInfoItemAsk" v-if="info.record.length<=index"></image>
					</view>
				</view>
				<view class="surplusUser" v-if="info.surplusUser!=0">仅剩<text>{{info.surplusUser}}</text>人，快呼唤小伙伴参加吧！</view>
				...
				<view class="" style="display: flex;margin-top: 30upx;justify-content: center;">
					...
					<view class="timeWrap">
						<u-count-down v-if="info.surplusTime||info.surplusTime===0" @end="endTime" :timestamp="info.surplusTime" :show-border="false" font-size="28" color="white" :bgColor="info.surplusTime===0?'#999999':'#F23A3A'" :separatorColor="info.surplusTime===0?'#999999':'#F23A3A'" :showDays="true" height="46"></u-count-down>
					</view>
				</view>
		...
		<view class="proWrap" v-if="!isJoin&&(info.record.length<info.num)">
			<view class="proItem" v-for="(item,index) in goodsList" :key="index" :data-index="index" @click="selectProduct">
				<view class="proItemT">
		...
		<view class="submit" @click="submit" v-if="!isJoin&&(info.record.length<info.num)">{{endFlag?'拼团已过期':'立即参团'}}</view>
...
<script>
	...
	export default {
		...
		onShareAppMessage() {
			return {
				title:"拼团详情",
				path:"/pages/collageDetail/collageDetail?groupid="+this.groupid,
			...
			getDetail(){
				...
				collageApi.groupInfo(obj).then(res=>{
					...
					if(res.data.detail.num <= res.data.detail.record.length){
						...
						res.data.detail.surplusTime = res.data.detail.expire_time - Math.floor(new Date().getTime()/1000);
					}
...
<style scoped>
	...
	.collageUserTitleL,.collageUserTitleR{
		display: flex;
		align-items: center;
	}
