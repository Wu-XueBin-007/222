<template>
  <view v-show="!isLoading" :class="['container',type=='seckill'?'':'containers']">
	  <seckill-nav title="商品详情" :fontSize="type=='product'?30:30" :backType="type" :color="type=='product'?'black':'#333333'"></seckill-nav>
    <!-- 商品图片轮播 -->
		...
		<view class="seckillTimeR">
			<view class="seckillTimeRWrap">
				...
				<view class="seckillTimeRB" style="display: flex;">
					<u-count-down v-if="true" style="display: flex;" @end="endTime" :timestamp="goods.surplusTime" :show-border="false" font-size="22" color="#FFFFFF" bgColor="#9C43FD" separatorColor="#9C43FD" :showDays="true" height="38"></u-count-down>
				</view>
			</view>
		</view>
	</view>
    ...
    <!-- 选择商品规格 -->
    <view v-if="goods.spec_type == 20 && type=='product'" class="goods-choice m-top20 b-f" @click="onShowSkuPopup(1)">
      <view class="spec-list">
        <view class="flex-box">
          <text class="spec-list-text">选择</text>
          <text class="spec-name spec-list-text2" v-for="(item, index) in goods.specList" :key="index">{{ item.spec_name }}</text>
        </view>
	...
	<view v-if="goods.spec_type == 20 && type=='seckill'" class="goods-choice m-top20 b-f" @click="onShowSkuPopup(3)" style="width: 718rpx;box-shadow: 0 2rpx 6rpx 0 #C9C9C9;border-radius: 12rpx;padding: 18rpx 0;margin: 0 auto;">
	  <view class="spec-list" style="padding: 0 24rpx;height: 100%;width: 100%;box-sizing: border-box;">
	    <view class="flex-box" style="display: flex;">
	      ...
	      <text class="spec-name spec-list-text2" v-for="(item, index) in goods.specList" :key="index" style="font-size: 28rpx;line-height: 40rpx;">{{ item.spec_name }}</text>
	    </view>
	    <view class="spec-arrow f-26 col-9 t-r">
			<image src="../../static/icon/icon_right2.png" mode=""></image>
	    </view>
    ...
    <!-- 商品SKU弹窗 -->
	<SkuPopup v-if="!isLoading" v-model="showSkuPopup" :skuMode="skuMode" :poolId="poolId" :bigId="bigId" :group_order_id="group_order_id" :LuckyFreeId="LuckyFreeId" :goods="goods" @addCart="onAddCart" :type="type" :status="timeStatus" />
	...
	<view class="operaBtn" v-if="type=='seckill'">
		...
		<view class="operaBtnR" @click="onShowSkuPopup(3)" :style="{'background':timeStatus==1?'#DD1010':'#999999'}">{{timeStatus==0?'未开始':(timeStatus==1?'立即抢购':'已结束')}}</view>
	</view>
		...
		getCommon() {
			...
			return new Promise((resolve, reject) => {
				settingApi.detail().then(res => {
					res.data.detail.seckill_content = res.data.detail.seckill_content.replace(/<img/g,"<img style='width:100%'");
      ...
      getGoodsDetail() {
        ...
        return new Promise((resolve, reject) => {
			if(app.type == 'product'){
				...
				seckillApi.goodsDetails({goodsId:app.goodsId,timeId:app.timeId})
				  .then(result => {
					  ...
					  let nowNum = Number(nowTime.getHours())*60*60 + Number(nowTime.getMinutes())*60 + Number(nowTime.getSeconds());
