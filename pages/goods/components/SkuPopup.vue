<template>
  <goods-sku-popup :value="value" @input="onChangeValue" border-radius="20" :custom-action="findGoodsInfo"
    ...
    @open="openSkuPopup" @close="closeSkuPopup" @add-cart="addCart" @buy-now="buyNow" :priceColor="type=='product'?'#EF343D':'#DD1010'" :buyNowBackgroundColor="type=='product'?'#EF343D':(((status==0||status==2)&&type=='seckill')?'#999999':'#DD1010')" :buyNowText="type=='seckill'?(status==0?'未开始':(status==1?'立即购买':'已结束')):'立即购买'" />
</template>
    ...
    methods: {
      ...
      buyNow(selectShop) {
		...
		if(this.type == 'product'){
			...
			uni.navigateTo({
				url:"/pageSecKill/sureOrder/sureOrder?mode=buyNow&goodsId="+selectShop.goods_id+"&goodsSkuId="+selectShop.goods_sku_id+"&goodsNum="+selectShop.buy_num+"&secid="+this.goods.seckill_id+"&timeid="+this.goods.timeId
			})
