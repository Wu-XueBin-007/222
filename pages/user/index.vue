	...
	<view class="head" :style="{'padding-top':navInfo.paddingTop+'px'}">
		...
		<view class="headT">
			<view class="imgWrap" @click="handleLogin">
				<view class="headView" :style="{'background-image':'url('+(userInfo.avatar_url ? userInfo.avatar_url : '/static/default-avatar.png')+')'}"></view>
				...
				<view class="" v-if="isLogin" style="display: flex;">
					...
					<view class="">
						<view class="userInfoB">
							...
							<view class="userInfoBR" v-if="isLogin" :data-id="userInfo.user_id" @click="copy">复制</view>
						</view>
					</view>
				</view>
				
			</view>
			<image src="../../static/mine/setting.png" @click="onTargetSetup" style="width: 40rpx;height: 40rpx;position: absolute;right: 30rpx;bottom: 12rpx;"></image>
			<!-- <view class="codeWrap" @click="toLink">
				<image src="../../static/mine/code.png"></image>
				<view>邀请好友</view>
			</view> -->
			<image src="../../static/mine/code.png" @click="toLink" style="width: 40rpx;height: 40rpx;position: absolute;right: 100rpx;bottom: 12rpx;"></image>
		</view>
		<view class="headB">
			<view class="headItem" @click="Tongzheng">
				<!-- <view class="headItemT">{{token_price?token_price:(isLogin?0:'---')}}</view> -->
				<view class="headItemT">{{userInfo.cert_total_price?userInfo.cert_total_price:(isLogin?0:'---')}}</view>
				<view class="headItemB">通证</view>
			</view>
			<view class="headItem" @click="toProfit">
				<view class="headItemT"><text>￥</text>{{userInfo.count_profit?userInfo.count_profit:(isLogin?0:'---')}}</view>
			...
			<view class="headItem" @click="onTargetWallet">
				...
				<view class="headItemB">我的钱包<image src="../../static/icon/more_gray.png"></image></view>
			</view>
		</view>
	</view>
	<view class="orderWrapBox" :style="{'position': 'relative','z-index': '999'}">
		...
		<view class="orderWrap" style="margin: -20rpx auto 0;">
			...
			<view class="orderWrapCon">
				<view class="orderItem" v-for="(item,index) in orderNavbar" :key="index" @click="onTargetOrder(item)">
	...
	z-index: 99;padding: 0 10rpx;font-size: 24rpx;text-align: center;">
		...
		<view class="orderWrap" style="margin-top: 30rpx;">
			<view class="orderWrapHead">
				<view  style="font-size: 30rpx;line-height: 28rpx;color: #000000;font-weight: 700;font-family: PingFang;">购票订单</view>
				<!-- <image src="../../static/icon/more_gray.png" class="orderWrapHeadR"></image> -->
			</view>
			<view class="orderWrapCon2">
				<view class="orderItem" v-for="(item,index) in orderTicket" :key="index" @click="onTargetOrders(item)">
					<view>
						<image :src="item.icon"></image>
					</view>
					<text>{{item.name}}</text>
		...
		<view class="orderWrap" style="margin-top: 30rpx;">
			<view class="orderWrapHead">
				<view  style="font-size: 30rpx;line-height: 28rpx;color: #000000;font-weight: 700;font-family: PingFang;">我的工具</view>
				<!-- <image src="../../static/icon/more_gray.png" class="orderWrapHeadR"></image> -->
			</view>
			<view class="orderWrapCon2">
...
<script>
  ...
  const orderNavbar = [
    ...
    { id: 'payment', name: '待付款', icon: '../../static/mine/waitPay.png', count: 0 },
    { id: 'received', name: '待发货', icon: '../../static/mine/daifahuo.png', count: 0 },
    { id: 'delivery', name: '待收货', icon: '../../static/mine/daishouhuo.png', count: 0 },
	{ id: 'comment', name: '待评价', icon: '../../static/mine/daipinjia.png', count: 0 },
	{ id: 'all', name: '售后', icon: '../../static/mine/shouhou.png' , count: 0 },

  ]
// 购票订单操作
  const orderTicket = [
    { id: '1', name: '待支付', icon: '../../static/mine/icon_ticket_wait.png', count: 0 },
    { id: '2', name: '待出票', icon: '../../static/mine/icon_ticket_out.png', count: 0 },
    { id: '3', name: '已出票', icon: '../../static/mine/icon_ticket_finish.png', count: 0 },
	{ id: 'all', name: '全部订单', icon: '../../static/mine/icon_ticket_pay.png' },
  ]
  // 活动专区
  const activityNavbar = [
	  {path:"/pageHome/myCollage/myCollage",name:"我的拼团",icon:"../../static/mine/pintuan.png"},
	  {path:"/pageHome/distribution/index",name:"我的团队",icon:"../../static/mine/tuandui.png"},
	  {path:"/pageHome/coupon/index",name:"领券中心",icon:"../../static/mine/lingquan.png"},
	  {path:"/pageHome/code/index",name:"邀请好友",icon:"../../static/mine/yaoqing.png"}
  ]
  /**
   更多服务
   ...
   */
  const service = [
    { id: 'distribution', name: '分销中心', icon: '../../static/user/fenxiao.png', type: 'link', url: 'pageHome/distribution/index' },
	{ id: 'myCoupon', name: '积分商城', icon: '../../static/user/jifen.png', type: 'link', url: 'pageHome/pointsMall/index' },
    { id: 'coupon', name: '领券中心', icon: '../../static/user/lingquan.png', type: 'link', url: 'pageHome/coupon/index' },
	{ id: 'integralCoupon', name: '优惠券兑换', icon: '../../static/user/lingquan.png', type: 'link', url: 'pageHome/coupon/integral' },
    { id: 'help', name: '我的卡券', icon: '../../static/user/kaquan.png', type: 'link', url: 'pageHome/cardVoucher/index' },
	{ id: 'address', name: '收货地址', icon: '../../static/user/dizhi.png', type: 'link', url: 'pages/address/index' },
	{ id: 'contact', name: '我的兑换', icon: '../../static/user/duihuan.png', type: 'link', url: 'pageHome/my-exchange/index' },
	// { id: 'code', name: '推广码', icon: '../../static/user/code.png', type: 'link', url: 'pages/code/index' },
	{ id: 'gift', name: '我的礼品', icon: '../../static/user/gift.png', type: 'link', url: 'pageHome/my-exchange/gift' },
	{ id: 'scanCode', name: '核销', icon: '../../static/user/scanCode.png', type: 'link', url: 'code' },
    // { id: 'contact', name: '在线客服', icon: 'kefu', type: 'button', openType: 'contact' },
    { id: 'refund', name: '我的充值卡', icon: '../../static/user/card.png', type: 'link', url: 'pageHome/teledcard/index' },
	{ id: 'refund', name: 'VIP卡券', icon: '../../static/user/vipCard.png', type: 'link', url: 'pageHome/vipCard/index' },
	{ id: 'refund', name: '个人仓库', icon: '../../static/user/duihuan.png', type: 'link', url: 'pageHome/vipCard/privilegeCard/index' },
  ]
  ...
  export default {
    ...
    methods: {
      ...
      // 跳转到订单页
      onTargetOrder(item) {
        this.$navTo('pageHome/order/index', { dataType: item.id })
      },
	// 跳转到订单页
      onTargetOrders(item) {
        this.$navTo('pageHome/orderTicket/index', { dataType: item.id })
      },
      // 跳转到我的积分页面
      onTargetPoints() {
      ...
      // 跳转到服务页面
      handleService({ url,type }) {
		  //console.log(url,type)
		if(url == "code"){
			uni.scanCode({
				success(res) {
					//console.log(res)
					OrderApi.ScanCodeReceipt({order_id:res.result})
						.then(res=>{
							//console.log(res)
							uni.showToast({
								icon:'none',
