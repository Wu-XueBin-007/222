<template>
  <view class="container p-bottom" v-if="order.goodsList.length">
      ...
      <view class="checkout_list" v-for="(item, index) in order.goodsList" :key="index">
        <view class="flow-shopList dis-flex" data-index="index" @click="onTargetGoods(item.goods_id)">
          ...
          <view class="flow-list-left">
            <image mode="scaleToFill" :src="options.mode=='integral'?item.images[0].file.preview_url:item.goods_image"></image>
          </view>
          <view class="flow-list-right flex-box">
            <!-- 商品名称 -->
            <text class="goods-name twolist-hidden">{{ options.mode=='integral'?item.name:item.goods_name }}</text>
            <!-- 商品规格 -->
            <view class="goods-props clearfix">
              <view class="goods-props-item" v-for="(props, idx) in item.skuInfo.goods_props" :key="idx" v-if="options.mode!='integral'&&options.mode!='gift'">
    ...
    <view class="pay-method flow-all-money b-f m-top20" v-if="options.mode!='integral'&&options.mode!='gift'">
      ...
      <view class="pay-item dis-flex flex-x-between" @click="handleSelectDelivery(DeliveryTypeEnum.EXPRESS.value)">
      ...
      <!-- 送货上门 -->
      <view class="pay-item dis-flex flex-x-between" v-if="isSHSM" @click="handleSelectDelivery(DeliveryTypeEnum.SELFMENTION.value)">
        <view class="item-left dis-flex flex-y-center">
	...
	<view class="pay-method flow-all-money b-f m-top20" style="padding: 40rpx;" v-if="is_free==1">
	  <view style="display: flex;justify-content: center;">
		...
		<image style="width: 80rpx;height: 80rpx;margin-left: 96rpx;" src="../../static/home/ask.png" mode=""></image>
		<image style="width: 80rpx;height: 80rpx;margin-left: 96rpx;" src="../../static/home/ask.png" mode=""></image>
		<image style="width: 80rpx;height: 80rpx;margin-left: 96rpx;" src="../../static/home/ask.png" mode=""></image>
	  </view>
    ...
    <!-- 积分说明弹窗 -->
    <u-modal v-model="showPoints" :title="`${setting.points_name}说明`" v-if="options.mode!='integral'&&options.mode!='gift'">
      ...
      <view class="popup__coupon">
        ...
        <view class="coupon-list">
          <scroll-view :scroll-y="true" style="height: 565rpx;">
            <view class="coupon-item1" v-for="(item, index) in order.couponList" :key="index" style="margin-bottom: 20upx;">
	...
	<u-popup v-model="showPopup1" mode="bottom" v-if="options.mode!='integral'&&options.mode!='gift'">
	  <view class="popup__coupon">
	    ...
	    <view class="coupon-list">
	      <scroll-view :scroll-y="true" style="height: 565rpx;">
	        <view :class="['coupon-item']" v-for="(item, index) in order.UserCardVoucherList.data" :key="index" @click="handleSelectCoupon1(index)">
  ...
  <view v-else-if="showModalStatus3">
	<view class="home-popup2" catchtouchmove="true" >
	 ...
	 <view style="background: #000000 ; width: 750upx; height: 1500upx; z-index: -99; opacity: 0.6; position: absolute; top: 0; left: 0; right: 0; bottom: 0;"></view>
  </view>
...
<script>
  ...
  import {orderAdd} from "@/api/integral/index.js"
    ...
    methods: {
		...
		nowExchange(){
			...
			if(this.options.mode == "integral"){
				...
				obj.data.receiving_area = this.order.address.region.province + this.order.address.region.city + this.order.address.region.region + " " + this.order.address.detail;
				...
				obj.form.area = this.order.address.region.province + this.order.address.region.city + this.order.address.region.region + " " + this.order.address.detail;
      ...
      navToMyOrder(order_on) {
        setTimeout(() => {
          this.$navTo('pages/cashier/index',{order_on})
        }, 1000)
      },
