<template>
	<view class="container">
		...
		<view v-if="list.length" class="cart-list">
			<view class="cart-item" v-for="(item, index) in list" :key="index" @click="onTargetGoods(item.goods_id)">
				<label class="item-radio" @click.stop="handleCheckItem(item.id)">
					<radio class="radio" color="#ff5060" :checked="inArray(item.id, checkedIds)" />
				</label>
		...
		<!-- 购物车数据为空 -->
		<empty v-if="!list.length" :isLoading="isLoading" :custom-style="{ padding: '180rpx 50rpx' }" tips="购物车空空如也哦~">
		...
		<view v-if="list.length" class="footer-fixed">
			<label class="all-radio" @click="handleCheckAll">
				<radio class="radio" color="#ff5060" :checked="true"
					v-if="checkedIds.length > 0 && checkedIds.length == list.length" />
				<radio class="radio" color="#ff5060" :checked="false" v-else />
				<text>全选</text>
			</label>
	...
	export default {
		...
		onShareAppMessage() {
			...
			return {
				...
				path: '/pages/cart/index?' + app.$getShareUrlParams()
			}
		},
		...
		methods: {
			...
			handleCheckItem(cartId) {
				...
				//console.log(index,cartId,this.checkedIds)

				index < 0 ? checkedIds.push(cartId) : checkedIds.splice(index, 1)
				this.checkedIds = checkedIds;
				//console.log(checkedIds.length > 0 && checkedIds.length == this.list.length)
			},
