<template>
	<view>
		<view class="All-goods">
			<view v-for="(item,index) in category" :key="index" :class="['possess',item.id==isActive?'possessMore':'']"
				@click="getActive(item.id)">{{item.name}}</view>
		</view>
			...
			<view class="goods-infos1" v-for="(item,index) in myrecordList" :key="index">
				...
				<view class="goods-right">
					...
					<view class="goods-right-3">
						...
						<view class="goods-right-02" v-if="isActive==1" @click="goodsNow" :data-index="index"
							:data-id="item.id">立即提货</view>
					...
					<view style="box-sizing: border-box;padding-bottom: 10rpx;">
						...
						<view class="detail">
							<view class="detail-top" @click="addRrea">
								...
								<view class="more">
									<image src="../../static/icon_right.png" class="moreImg"></image>
								</view>
							</view>
						...
						<view class="nShier-item">
							...
							<view class="nShier-item-list nShier-color" @click="btn_payTa(PayTypeEnum.WECHAT.value)">
								...
								<view class="coupons-item-chbox">
									<checkbox class="checks1-h5" :checked="paymentType==PayTypeEnum.WECHAT.value">
									</checkbox>
								</view>
							</view>
							<view class="nShier-item-list" @click="btn_payTa(PayTypeEnum.BALANCE.value)">
								...
								<view class="coupons-item-chbox">
									<checkbox class="checks1-h5" :checked="paymentType==PayTypeEnum.BALANCE.value">
									</checkbox>
								</view>
							</view>
						</view>
			...
			init(flag) {
				...
				boxApi.recordList(obj).then(res => {
					...
					if (res.status == 200) {
						...
						if (Math.ceil(res.data.record_list.total / _that.limit) != this.page && res.data
							.record_list.last_page > 0) {
							this.bottomFlag = true;
